...
function Component(options) {
    ...
    this.name     = options.isRoot // TODO deprecate 'self' name
        ? 'self'
        : this.json.name || path.basename(options.dir).replace("bower-asset/", "");
    ...
    console.log("Dependencies for ", this.name, this.dependencyMap);
...
function getDependentComponents(dependencies, options) {
    ...
    return Object.keys(dependencies).map(function (key) {
        if (!createdComponents.hasOwnProperty(key)) {
            ...
            if (options.overrides._packageNames && options.overrides._packageNames.hasOwnProperty(key)) {
                componentDirectory = path.resolve(options.dependencyDir, options.overrides._packageNames[key]);
            } else {
                componentDirectory = path.resolve(options.dependencyDir, key);
            }
