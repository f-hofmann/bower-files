    ...
    if (!cl) {
        ...
        var files = filehound.create()
            .addFilter(function (file) {
                    var bn = path.basename(file._pathname);
                    if (bn == 'composer.json' || bn == 'bower.json' || bn == 'component.json') {
                        return true;
                    }
            ...
            .paths(options.dependencyDir)
            .findSync();

        files = unique(files.map(function (k) {
            return path.dirname(k);
        }));

        for (var i in files) {
            var componentDirectory    = files[i];
            var relComponentDirectory = componentDirectory.replace(options.dependencyDir + "/", "");

            var bower     = readAJson(path.resolve(componentDirectory, "bower.json"));
            var composer  = readAJson(path.resolve(componentDirectory, "composer.json"));
            var component = readAJson(path.resolve(componentDirectory, "component.json"));


            var componentData = {
                "name"    : bower.name || composer.name || component.name || relComponentDirectory,
                "fullpath": componentDirectory
            }

            if (bower && bower.name) {
                cl[bower.name] = componentDirectory;
            }

            if (composer && !cl.hasOwnProperty(composer.name)) {
                cl[composer.name] = componentDirectory;
            }

            if (component && !cl.hasOwnProperty(component.name)) {
                cl[component.name] = componentDirectory;
            }

            if (relComponentDirectory && !cl.hasOwnProperty(relComponentDirectory)) {
                cl[relComponentDirectory] = componentDirectory;
            }

            var shortDir = path.basename(relComponentDirectory);

            if (!cl.hasOwnProperty(shortDir)) {
                cl[shortDir] = componentDirectory;
            }
        }
    }
...
function getDependentComponents(dependencies, options) {
    ...
    return Object.keys(dependencies).map(function (key) {
        if (!createdComponents.hasOwnProperty(key)) {
            var cl = getComponentList(options);

            if (!cl.hasOwnProperty(key)) {
                console.warn("Komponente", key, "nicht vorhanden");

                createdComponents[key] = new Component(assign({}, options, {
