...
FileFilter.prototype = {
    ...
    getSortedDependencies: function () {
        var self         = this._filter();
        var dependencies = self.getDependencies();

        var map = [];

        dependencies.map(function (comp) {
            comp.dependencyMap.map(function (entry) {
                map.push(entry);
            })
        });
        ...
        toposort(map).reverse().map(function (componentName) {
            dependencies.map(function (dep) {
                if (dep.name === componentName) {
                    sortedDependencies.push(dep);
                }
            });
        });
...
Object.defineProperties(FileFilter.prototype, {
    files    : {
        get: function () {
            ...
            var files = [];
            self.getSortedDependencies().map(function (component) {
                if (self.options.verbose) {
                    console.log("Component " + component.name);
                    console.log("  provides: ");
                }

                component.files.map(function (e) {
                    if (self.options.verbose) {
                        console.log("       " + e);
                    }
                    files.push(e);
    ...
    deps     : {
        get: function () {
            var self    = this._filter();
            var options = self.options;
            return self.getSortedDependencies().reduce(function (finalDeps, component) {
    ...
    depsArray: {
        get: function () {
            var self    = this._filter();
            var options = self.options;
            return self.getSortedDependencies().map(function (component) {
                return {
